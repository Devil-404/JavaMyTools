...
import com.zyd.tools.excel.factory.CustomExcelStyle;
import com.zyd.tools.excel.factory.EasyExcelWriterFactory;
...
/**
 * @description:使用easyExcel编写的工具类
 * @projectName:MyTool
 * @see:com.zyd.tools
 * @author:张屹东
 * @createTime:2020/9/28 20:47
 */
public class ExcelUtilsV2 {

    /**
     * @param fileStream
     * @param rowNumber  表头占的行数，从0开始（如果要连表头一起读出来则传0）
     * @return
     * @throws Exception
     * @deprecated 已弃用
     ...
     */
    @Deprecated
    public static List<Object> read(InputStream fileStream, int rowNumber) throws Exception {
        List<Object> data = EasyExcelFactory.read(fileStream, new Sheet(1, rowNumber));
     ...
     * @return
     */
    public static List<Map<Integer, Object>> read(String filePath, int headRowNum) {
        return EasyExcelFactory.read(filePath).sheet().headRowNumber(headRowNum).doReadSync();
    }
     ...
     * @param num      表头单元格数量
     */
    public static void write(String sheetName, String filePath, List<String> head, List<List<Object>> data, int num) {
        EasyExcel.write(filePath).sheet(sheetName).registerWriteHandler(new CustomExcelStyle(num, head)).relativeHeadRowIndex(1).doWrite(data);
    }
    ...
    public static void writeTemplate(String filePath, String templateFileName, List<?> data) {
        EasyExcel.write(filePath).withTemplate(templateFileName).sheet().useDefaultStyle(true).doWrite(data);
    }
